...
- **定理意义：** 理解为什么策略梯度不需要对环境动力学（状态分布）求导，这是该定理最关键的贡献。
...
- **简答题:**
    ...
    - **2021-2022 Q3b:** 解释策略梯度解决了什么“复杂性”问题（即无法对未知环境动力学求导），以及策略梯度定理如何提供解决方案（将梯度重写为只与策略和动作价值有关的期望形式）。
...
- **基线 (Baseline):** 引入基线（如 $V(s)$）可以显著降低方差，且**不会引入偏差**。
...
- **简答/分析题:**
    ...
    - **2024-2025 Q3c:**
        ...
        - (iii) 解释 REINFORCE 为什么方差大，以及列举两种降低方差的方法（基线、Actor-Critic）并解释原理。
...
- **结合值函数：** Actor-Critic 使用 Critic（值函数）来引导 Actor（策略）更新，通过自举（Bootstrapping）引入偏差但显著降低方差。
...
- **DDPG:** 适用于连续动作空间，结合了 DQN 的经验回放和目标网络，本质上是确定性策略梯度 (DPG) 的深度版本。
    
- **连续动作：** 策略梯度天然适合连续动作空间（如机器人控制），而基于值的 DQN 难以处理。
